namespace Terrasoft.Core.Process
{
	...
	#region Class: TaskCalculateAveragePriceProcessMethodsWrapper

	/// <exclude/>
	public class TaskCalculateAveragePriceProcessMethodsWrapper : ProcessModel
	{

		public TaskCalculateAveragePriceProcessMethodsWrapper(Process process)
			: base(process) {
		...
		private bool ScriptTask1Execute(ProcessExecutingContext context) {
			var esq = new EntitySchemaQuery(UserConnection.EntitySchemaManager, "TaskRealty");
			var priceColumn = esq.AddColumn("TaskRealtyPriceUSD"); // SELECT UsrPriceUSD as UsrPriceUSD, UsrArea as UsrArea FROM UsrRealtyFreedomUI WHERE ...
			var areaColumn = esq.AddColumn("TaskRealtyArea");
			...
			var offerTypeFilter = esq.CreateFilterWithParameters(FilterComparisonType.Equal, "TaskRealtyOfferType", offerTypeId);
